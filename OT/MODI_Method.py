...
def calculate_ui_vi(cost_table,ans_table):
    ...
    row_count = len(ans_table)       # count of rows and columns of the ans_table
    col_count = len(ans_table[0])

    U = {}                           # dictionaries to store ui and vi values 
    V = {}                           # calculated from the ans_table
   
    U['u0'] = 0                      # setting a intial value u0 = 0 in dictionary U
                                     # to calculate all ui and vi values
     
    # intializing first row to be iterated to set up vj value for the columns 
    # of allocated cell in the ans_table[0] row
    row_indices_for_vj_calc   = [0]  # setting up list to iterate through 
    column_index_for_uj_calc  = []   # row and column with uj and vj value 
                                     # to calculate vj and uj respectivley

    {# iterating through each row with index in the list 
    # row_indices_for_vj_calc to go through each column
    # and allocate vj if not allocated
    # col is the column index and allocation is the value
    # in ans_table if its zero then not allocated else allocated  
    # should allocate vj and uj into U and V till the length of 
    # U and V reach the row or column count
    }
    while(len(U) != row_count or len(V) != col_count):
        
        for row in row_indices_for_vj_calc:                           # check_if_present will be True if V already as the column
            for col,allocation in enumerate(ans_table[row]):          # allocated with a vj value else it will be False
            
                check_if_present = f'v{col}' in V
                if(allocation!= 0 and check_if_present == False):
                    
                    column_index_for_uj_calc.append(col)                # if the cell of the column has a non zero value then
                    V[f'v{col}'] = cost_table[row][col] -U[f'u{row}']   # it is one of the m+n -1 basic variable and if it is 
                                                                        # not present in V then it is added to V and vj calculated
                    
        row_indices_for_vj_calc = []               # resetting row_indices_for_vj_calc to empty set
                                                   # to calculate new rows by going through columns
        # uncomment for detailed explanation
        # print("column indices for uj alocation : ",column_index_for_uj_calc)
        # print("modified V from the iteration   :",V)
        #print(" ")
        
        for index in column_index_for_uj_calc:
            for pos in range (len(ans_table)):
                check = f'u{pos}' in U
                if(ans_table[pos][index]!=0 and check == False):
                    row_indices_for_vj_calc.append(pos)
                    U[f'u{pos}'] = cost_table[pos][index] -V[f'v{index}']
        column_index_for_uj_calc = []
        print(row_indices_for_vj_calc,U)

    print(U,V)

{# def MODI_METHOD(cost_table,ans_table):

#     # dictionaries to store ui and vi values 
#     # calculated from the ans_table
#     U = {}
#     V = {}

#     # setting a intial value u0 = 0
#     # to calculate all ui and vi values
#     row_index = [0]
#     U['u0']   =  0
#     column_index = []
...
calculate_ui_vi(cost_table,ans_table)
